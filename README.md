...
### CyrusSasl
This simple role will install [cyrus-sasl](https://www.archlinux.org/packages/extra/x86_64/cyrus-sasl/) and configure SASL to use LDAP with TLS. The current SASL environment uses 3 replicated LDAP hosts defined in `saslauthd.conf` and can be fine-tuned for other LDAP environments.
